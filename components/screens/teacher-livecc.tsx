...
export default function TeacherLiveCC({ setActive }: Props) {
  ...
  const [studentCount, setStudentCount] = useState(0); // 💥 New Roster State

  useEffect(() => {
    if (!token) return;

    let wsUrl = API_BASE_URL.replace(/^http/, "ws").replace(/\/api\/?$/, "/ws/cc");
    ...
    ws.onmessage = (event) => {
      try {
        ...
        // 💥 MAGIC: Detect if it's a roster update or a chat message
        ...
        console.log("Error parsing websocket");
      }
    };
